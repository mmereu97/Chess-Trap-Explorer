...
class PGNImportService:
    ...
    def import_from_folder(self, folder_path: str, max_moves: int = 25, checkmate_only: bool = False) -> Tuple[int, int]:
        """Import traps from all PGN files in a folder."""
        total_white = 0
        total_black = 0
        ...
        for pgn_file in pgn_files:
            try:
                print(f"[DEBUG FOLDER] Processing: {pgn_file.name}")
                # Și aici pasăm noul parametru
                white_count, black_count = self.import_from_file(str(pgn_file), max_moves, checkmate_only)
    ...
    def _parse_pgn_file(self, file_path: str, max_moves: int, checkmate_only: bool) -> Tuple[List[ChessTrap], List[ChessTrap]]:
        """Final, strict parser. Only imports lines that verifiably end in checkmate."""
        print(f"[DEBUG PGN PARSE] Opening file with STRICT-CHECKMATE method: {file_path}")
        ...
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as pgn_file:
            ...
            while True:
                try:
                    ...
                    if not (4 <= len(moves) <= max_moves): continue
                    ...
                    # --- FILTRU STRICT ---
                    # Verificăm atât pe tablă, CÂT ȘI notația SAN
                    if not board.is_checkmate() or not san_moves[-1].endswith('#'):
                        continue

                    # Dacă am trecut de filtru, știm sigur că e mat
                    num_moves = len(san_moves)
                    trap_color = chess.WHITE if (num_moves % 2 != 0) else chess.BLACK
                    
                    trap_name = game.headers.get("Event", f"Imported Trap {processed_games}") + " (Checkmate)"
                    trap = ChessTrap(name=trap_name, moves=san_moves, color=trap_color)

                    if trap_color == chess.WHITE: white_traps.append(trap)
                    else: black_traps.append(trap)
                except:
                    continue
                
        print(f"[DEBUG PGN PARSE] Found: {len(white_traps)} white traps, {len(black_traps)} valid checkmate traps.")
