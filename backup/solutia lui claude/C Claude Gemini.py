...
class PGNImportService:
    ...
    def _process_games_chunk(game_strings: List[str], max_moves: int, checkmate_only: bool) -> Tuple[List[ChessTrap], List[ChessTrap]]:
        ...
        # Importurile necesare trebuie făcute în interiorul funcției, 
        # deoarece aceasta rulează într-un proces complet nou și separat.
        import chess.pgn
        import io
        import psutil  # Import pentru a controla procesul
        import os      # Import pentru a obține ID-ul procesului

        # --- ÎNCEPUTUL MODIFICĂRII PENTRU PRIORITATE ---
        # Fiecare proces muncitor care execută această funcție își va seta propria prioritate.
        try:
            current_process = psutil.Process(os.getpid())
            # Setăm prioritatea la 'Above Normal'. Este un echilibru bun:
            # - Spune sistemului de operare că este o sarcină importantă, favorizând P-Cores.
            # - Nu este la fel de agresiv ca 'High', care ar putea face sistemul să nu mai răspundă.
            current_process.nice(psutil.ABOVE_NORMAL_PRIORITY_CLASS)
            # Următorul print este opțional, dar util pentru a confirma că setarea a funcționat.
            # Va apărea în consolă pentru fiecare proces muncitor.
            # print(f"[DEBUG WORKER PID:{os.getpid()}] Priority set to Above Normal.")
        except Exception as e:
            # Prindem orice eroare care ar putea apărea la setarea priorității (ex: permisiuni).
            print(f"[DEBUG WORKER PID:{os.getpid()}] WARNING: Failed to set process priority: {e}")
        # --- SFÂRȘITUL MODIFICĂRII PENTRU PRIORITATE ---
        ...
        for game_string in game_strings:
            try:
                # Parsează jocul din string-ul PGN.
                # Folosim io.StringIO pentru a trata string-ul ca pe un fișier în memorie.
                game = chess.pgn.read_game(io.StringIO(game_string))
                if game is None:
                    # Dacă parsarea eșuează sau string-ul nu este un joc valid, trecem la următorul.
                    continue
                
                # Extragem lista de mutări din joc.
                moves = list(game.mainline_moves())
                
                # Aplicăm filtrul pentru numărul de mutări (semi-mutări).
                if not (4 <= len(moves) <= max_moves):
                    continue
                
                # Reconstruim tabla de la zero și generăm mutările în format SAN (Standard Algebraic Notation).
                board = chess.Board()
                san_moves = []
                for move in moves:
                ...
                # Verificare strictă: ultima mutare trebuie să fie șah-mat.
                # Verificăm atât starea tablei (`is_checkmate()`) cât și notația ('#').
                if not board.is_checkmate() or not san_moves[-1].endswith('#'):
                    continue
                
                # Determinăm culoarea care a dat matul.
                # Dacă numărul de mutări este impar, a câștigat Albul.
                # Dacă este par, a câștigat Negrul.
                num_moves = len(san_moves)
                trap_color = chess.WHITE if (num_moves % 2 != 0) else chess.BLACK
                
                # Creăm numele capcanei folosind header-ul 'Event' din PGN.
                trap_name = game.headers.get("Event", f"Imported Trap") + " (Checkmate)"
                trap = ChessTrap(name=trap_name, moves=san_moves, color=trap_color)
                
                # Adăugăm capcana la lista corespunzătoare culorii.
                ...
                # O eroare generală pentru a prinde orice problemă neașteptată cu un joc
                # (ex: format PGN corupt, mutare ilegală etc.) și a preveni blocarea procesului muncitor.
                # Trecem la următorul joc în tăcere.
                continue
        
        # La final, procesul muncitor returnează cele două liste de capcane găsite în "chunk"-ul său.
        return white_traps, black_traps
